...
@section('content')
    <h3 class="mt-4">Update Masa Kerja</h3>
    <div class="card mb-4">
        <div class="card-body">
            <form action="/update-masakerja/{{ $dataM->id }}" method="POST">@csrf
                <div class="mb-3"><label class="form-label">Karyawan</label>
                    <select class="form-select" name="karyawan_id" required>
                        @foreach ($karyawan as $k)<option value="{{ $k->karyawan_id }}" {{ $k->karyawan_id == $dataM->karyawan_id ? 'selected' : '' }}>{{ $k->nama_tk }}</option>@endforeach
                    </select>
                </div>
                <div class="mb-3"><label class="form-label">Tunjangan Masa Kerja</label><input type="number"
                        class="form-control" name="tunjangan_masakerja" value="{{ $dataM->tunjangan_masakerja }}" required>
                </div>
                <div class="mb-3"><label class="form-label">Tanggal Mulai</label><input type="date" class="form-control"
                        name="beg_date" value="{{ $dataM->beg_date }}"></div>
