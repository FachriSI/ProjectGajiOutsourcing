...
@section('title', 'Tambah UMP / UMK')

@section('content')

    <!-- Modern Page Header -->
    <div class="bg-white p-4 rounded shadow-sm mb-4 mt-4 border-start border-primary border-5">
        <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-money-bill-wave me-2 text-primary"></i> Tambah UMP / UMK</h1>
            ...
            <div class="card shadow-sm border-0 mb-4 border-top border-primary border-4">
                <div class="card-body">
                    <h5 class="card-title text-primary fw-bold mb-4">
                        <i class="fas fa-plus-circle me-2"></i>Data UMP / UMK Baru
                    </h5>
                    ...
                    <form class="form-horizontal form-bordered" method="post" enctype="multipart/form-data"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('kode_lokasi') is-invalid @enderror" name="kode_lokasi" id="kode_lokasi" required>
                                <option value="" selected disabled>Pilih Lokasi</option>
                                @foreach ($data as $item)
                                    <option value="{{$item->kode_lokasi}}" {{ old('kode_lokasi') == $item->kode_lokasi ? 'selected' : '' }}>{{$item->lokasi}}</option>
                                @endforeach
                            </select>
                            @error('kode_lokasi')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control uang @error('ump') is-invalid @enderror" name="ump" id="ump" placeholder="0" value="{{ old('ump') }}" required>
                            </div>
                            @error('ump')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('tahun') is-invalid @enderror" name="tahun" id="tahun" required>
                                ...
                                @for ($i = $startYear; $i <= $endYear; $i++)
                                    <option value="{{ $i }}" {{ (old('tahun') == $i || $i == $currentYear) ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                            @error('tahun')
