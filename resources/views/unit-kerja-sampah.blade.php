...
@section('content')
  ...
  <div class="bg-white p-4 rounded shadow-sm mb-4 mt-4 border-start border-danger border-5">
    <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-trash-alt me-2 text-danger"></i> Sampah Unit Kerja</h1>
    <p class="text-muted small mb-0 mt-1">Daftar data unit kerja yang telah dihapus (Archive)</p>
  </div>
  ...
  <div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
      <h6 class="m-0 font-weight-bold text-danger"><i class="fas fa-archive me-2"></i>Arsip Unit Kerja</h6>
      <a href="/unit-kerja" class="btn btn-outline-secondary btn-sm">
          ...
          <tbody>
            @foreach ($data as $item)
              <tr>
                ...
                <td class="fw-bold text-dark">{{ $item->unit_kerja }}</td>
                <td><span class="badge bg-secondary">{{ $item->deleted_by }}</span></td>
                <td class="small">{{ $item->deleted_at }}</td>
                <td class="text-center">
                  <a href="{{ url('restore-unit', $item->unit_id) }}" class="btn btn-sm btn-outline-primary"
